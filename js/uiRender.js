...
export function updateSimpleMobItem(item, mob) {
  ...
  if (timeEl) {
    timeEl.innerHTML = `<div class="grid items-center w-full h-full" style="grid-template-columns:18px 92px;gap:0;"><span class="timer-label w-full text-[14px] text-right opacity-90">${label}</span><span class="timer-value font-bold text-[14px] text-left ${isSpecialCondition ? 'label-next' : ''} ${isTimeOver ? 'time-over' : ''}">${timeValue}</span></div>`;
  }
