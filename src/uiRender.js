...
                    <div class="px-2 text-left repop-remaining-text whitespace-nowrap">${mob.repopInfo?.remainingStr || ""}</div>
                    <div class="text-center repop-percent-text whitespace-nowrap">${mob.repopInfo?.elapsedPercent?.toFixed?.(0) || 0}%</div>
                    <div class="px-2 text-right repop-next-time-text whitespace-nowrap">Next: ${mob.repopInfo?.nextMinRepopDate ? new Intl.DateTimeFormat('ja-JP', absFmt).format(mob.repopInfo.nextMinRepopDate) : "未確定"}</div>
        </div>
      </div>
    </div>
`;
