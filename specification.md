...
並列実行により、JSONデータの展開前にFirestoreから最新データが届く可能性がある。この整合性を保ちつつ、描画速度を極限まで高めるため以下の戦略をとる。

- **バッファリング**: JSON展開前のリアルタイムデータは `pending` バッファに一時保存する。
- **段階的計算（Deferred Calculation）**:
  - **フェーズ1 (最速表示)**: 画面表示に直結する「最新の討伐日時」と「メンテナンス時刻」のみを用いてリリポップ時間を暫定計算し、即座にリストをソート・描画する。
  - **フェーズ2 (バックグラウンド)**: UI表示開始後、天候や月齢などの重い「特殊条件計算」を Web Worker でバックグラウンド実行し、結果が出次第、順次UIを更新する。
- **操作保護**: すべての必須データ（基本データ、最新ステータス、メンテ情報）が統合されるまでローディング画面を維持し、不完全なデータに基づく誤操作を防止する。
...
2. **検証コード**: クライアントでランダムな `HUNT-XXXXXXXX`（base36による英数字8桁）コードを生成。
...
- 討伐時間からの経過秒数に基づき算出。
- メンテ明け（サーバー起動時刻）が前回の討伐時刻より新しい場合、リリポップ時間を通常比 **60%** に短縮（※**Fランクは対象外**であり、常に100%として計算）。
...
### 6.3 通知ロジック (`notificationManager.js`)

- **ブラウザ通知**: Web `Notification API` を使用。
- **通知条件**:
  - 湧き開始時刻の2分前、および開始時刻に到達した際に発報。
  - 重複発報を防ぐため、モブIDと湧き時刻を組み合わせた `notifiedCycles` (Set) で状態を管理。
- **オーディオ再生**: `Audio` オブジェクトを使用。ブラウザのオートプレイ制限を回避するため、通知スイッチをオンにする際のユーザーインタラクション時に `audio.play()` 行いボリューム0で再生させることで、以降の自動再生権限を確保している。

### 6.4 ソートアルゴリズム (`mobSorter.js`)

表示順序は以下の優先度に従って厳密に決定される。

1. **ステータスグループ**: `Time Over` > `Pop Window` > `Before Respawn` > `Maintenance` の順。
2. **グループ内順序 (Time Over)**:
    - **湧き条件**: 現在条件を満たしている（`isInConditionWindow`）ものを優先。
    - **ランク**: `S` > `F` > `A` の順。
    - **拡張パッケージ**: 新しい拡張（`ExpansionId` 降順）を優先。
    - **識別子**: `MobNo` および `Instance` 番号の昇順。
3. **グループ内順序 (その他)**:
    - **湧き条件**: 現在条件を満たしている（`ConditionActive`）ものを最優先。
    - **進行度**: `elapsedPercent` の降順（％が大きいほど上）。
    - **最短リリポップ**: `minRepop` の昇順。
    - **ランク**: `S` > `A` > `F` の順。
    - **拡張パッケージ**: 新しい拡張の降順。
    - **識別子**: `MobNo` および `Instance` 番号の昇順。
