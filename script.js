...
 * @param {string} rank - フィルタリングするランク
 * @param {boolean} shouldRestoreState - 画面の状態（スクロール位置、開いているカード）を復元するかどうか
 */
function renderMobList(rank, shouldRestoreState = false) {
    currentFilter = rank;

    // 状態復元のためのデータ収集
    let scrollPosition = 0;
    let openCardMobNo = null;
    
...
        if (openCard) {
            openCardMobNo = openCard.dataset.mobno;
        }
    }
    
...
        // 開いていたカードの再展開
        if (openCardMobNo) {
            const cardToRestore = document.querySelector(`.mob-card[data-mobno="${openCardMobNo}"]`);
            
            if (cardToRestore) {
                const panel = cardToRestore.querySelector('.expandable-panel');
                if (panel) {
                    // トランジションを無効化して瞬時に展開
                    panel.style.transition = 'none'; 
                    toggleMobDetails(cardToRestore);
                    
                    // 復元後、トランジションを元に戻す
                    setTimeout(() => {
...
    // レコード取得前に一度レンダリングし、loading state をクリア
...
    // ----------------------------------------------------
    
    // NEW: 状態復元フラグを決定（自動更新時のみ復元）
    const shouldRestoreState = (updateType === 'auto'); 
...
            
            // レンダリング時に状態復元フラグを渡す
            renderMobList(currentFilter, shouldRestoreState);
        } else {
...
    // --- 修正箇所: 討伐記録の定期更新を 10分ごと に戻す (10分 * 60秒 * 1000ミリ秒) ---
    setInterval(() => fetchRecordsAndUpdate('auto', false), 10 * 60 * 1000);

    // --- プログレスバーの定期更新は 60秒ごと で維持 ---
