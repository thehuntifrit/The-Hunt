...
/**
 * フィルターに基づいてモブカードリストをレンダリングする (省略 - 変更なし)
 */
...
async function submitReport() {
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            // 失敗時: ボタンを「送信失敗」の赤色に変え、モーダルは閉じない
...
/**
 * GASから最新の討伐記録と湧き潰し状態を取得し、グローバルデータを更新する (省略 - 変更なし)
 */
 ...
 * 修正: 以前の安定版のロジック (クラスを丸ごと再設定) に戻しています。
 */
function updateProgressBars() {
    ...
    document.querySelectorAll('.mob-card').forEach(card => {
        ...
        const progressBarEl = card.querySelector('.progress-bar'); // HTMLではクラスを最低限にしている
        ...
        if (progressBarEl) {
            ...
            // これにより、absolute, inset-0, rounded-xl などの基本スタイルが確実に適用されます。
