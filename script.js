...
 * @returns {number} ソートに使用する数値 (値が大きいほど優先度が高い)
 */
function getMobSortValue(mob) {
    // FATEモブはソート対象外とし、元のNo.順に戻す
    if (mob.Rank === 'F') {
        return -mob['No.']; 
    }
...
    
    // 1. リポップ時刻が不明なモブは、一番最後に表示するため負の大きな値とする
    if (repopData.isUnknown) {
        return -1000000;
    }
    
    // 2. リポップ期間に入っているモブ (isPop=true) を優先する
    let basePriority = repopData.isPop ? 10000 : 0;
    
    // 3. 経過パーセンテージを加えて、期間内の場合はパーセンテージが高いほど上位にする
    //    期間前の場合は、リポップ開始時刻 (minRepop) が早いほど上位にする
    if (repopData.isPop) {
        basePriority += repopData.elapsedPercent; // 0〜100
    } else {
        // 未POP期間: minRepopTime が現在時刻に近いほど大きな値
        const now = new Date().getTime();
        const minRepopTimeMs = repopData.minRepop.getTime();
        // キルタイムからの経過時間を使用
        const elapsedSinceKill = now - lastKillDate.getTime();
        // 値を小さくスケーリング
        basePriority += elapsedSinceKill / 1000000; 
    }
...
    // ソート値の降順 (高い優先度順) でソートする
    mobs.sort((a, b) => {
        const valueA = getMobSortValue(a);
        const valueB = getMobSortValue(b);

        if (valueB === valueA) {
            // ソート値が同じ場合、モブNo.で昇順 (古いモブ優先)
            return a['No.'] - b['No.'];
        }
...
    }
    
    // 1. ソート処理: 全てのランクでPOP時間に基づいてソート
    sortMobs(filteredMobs);
